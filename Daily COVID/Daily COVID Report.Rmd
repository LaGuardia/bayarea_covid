...
```{r table1, echo=FALSE}
   a1 %>% select(name,date,cases) %>% filter(name %in% c("Martinez","Pleasant Hill","Moraga","Orinda","Walnut Creek","Concord","Richmond","Lafayette"), date > (Sys.Date() - 8)) %>%
...
```{r table2, echo=FALSE}
   a1 %>% select(name,date,case_by_10k) %>% filter(name %in% c("Martinez","Pleasant Hill","Moraga","Orinda","Walnut Creek","Concord","Richmond","Lafayette"),date > (Sys.Date() - 8)) %>% 
...
a1 %>% filter( id %in% c("Pleasant Hill","Martinez","Lafayette")) %>% ggplot(aes(date,cases)) + 
